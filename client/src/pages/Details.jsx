...
import { get_product, get_banners } from "../store/reducers/homeReducer";
import {
  add_to_card,
  messageClear,
  add_to_wishlist,
...
import BulkPricingComponent from "./BulkPricingComponent";

const Details = ({ sellerId }) => {
  const navigate = useNavigate();
  const { slug } = useParams();
  const dispatch = useDispatch();
  const { product, relatedProducts, moreProducts, banners } = useSelector(
    (state) => state.home
  );
  ...
  const add_card = () => {
    if (userInfo) {
      const selectedAddonOptions = Object.values(selectedAddons).filter(
        (option) => option.price > 0
      );

      console.log(currentPrice, quantity, currentPrice * quantity);
  ...
  useEffect(() => {
    dispatch(get_product({ sellerId: sellerId, slug: slug }));
  ...
  const calculatePrice = () => {
    let basePrice =
      product.discount !== 0
        ? currentPrice - Math.floor((currentPrice * product.discount) / 100)
        : currentPrice;

    let addonsPrice = Object.values(selectedAddons).reduce(
      (total, option) => total + (option ? option.price : 0),
      0
    );
      ...
      <div className="bg-slate-100 py-5 mb-5">
        <div className="w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto">
          <div className="flex justify-start items-center text-md text-slate-600 w-full">
            ...
            <Link to={`/${sellerId}`}>{product.category}</Link>
            <span className="pt-1">
              <MdOutlineKeyboardArrowRight />
            </span>
          ...
          <div className="grid grid-cols-2 md-lg:grid-cols-1 gap-8">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="text-2xl text-red-500 font-bold flex gap-3">
                {product.discount !== 0 ? (
                  <>
                    ...
                    <h2>
                      {product.currency}
                      {calculatePrice()} (-{product.discount}%)
                    </h2>
                  </>
                ) : (
                  <h2>
              ...
              <div className="flex gap-3">
                ...
                <Link
                  to={`/${sellerId}/dashboard/chat/${product.sellerId}`}
          ...
          <div className="flex flex-wrap">
            <div className="w-[72%] md-lg:w-full">
              <div className="pr-4 md-lg:pr-0">
                ...
                <div>
                  {state === "reviews" ? (
                    <Reviews product={product} sellerId={sellerId} />
                  ) : (
