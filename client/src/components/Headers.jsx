...
import { get_support_info } from "../store/reducers/authReducer";

const Headers = ({ sellerId }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { categorys } = useSelector((state) => state.home);
  const { userInfo, supportInfo } = useSelector((state) => state.auth);
  ...
  const search = () => {
    navigate(
      `/${sellerId}/products/search?category=${category}&&value=${searchValue}`
    );
  };
  const redirect_card_page = () => {
    if (userInfo) {
      navigate(`/${sellerId}/card`);
    } else {
      navigate(`/${sellerId}/login`);
    }
  };
  useEffect(() => {
    if (sellerId) {
      dispatch(get_support_info(sellerId));
    }
  }, [dispatch, sellerId]);
  useEffect(() => {
    if (userInfo) {
  ...
  return (
          ...
          <div className="flex w-full justify-between items-center h-[50px] text-slate-500">
            <ul className="flex justify-start items-center gap-8">
              <li className="flex relative justify-center items-center gap-2 text-sm after:absolute after:h-[18px] after:w-[1px] after:bg-[#afafaf] after:-right-[16px]">
                ...
                <span>{supportInfo.email || "sourabhshail@gmail.com"}</span>
              </li>
            ...
            <div>
              <div className="flex justify-center items-center gap-10">
                ...
                <div className="relative flex group cursor-pointer text-slate-800 text-sm justify-center items-center gap-1">
                  <img
    ...
    <MdOutlineKeyboardArrowDown />
                  ...
                  <div className="relative after:h-[18px] after:w-[1px] after:bg-[#afafaf] after:absolute after:-right-[16px] before:absolute before:h-[18px] before:bg-[#afafaf] before:w-[1px] before:-left-[20px]">
                    <ul className="absolute opacity-0 transition-all ease-in-out duration-200 transform -translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 bg-black text-white p-2 w-[100px] flex flex-col gap-3 rounded-sm z-20">
                      <li>English</li>
                    </ul>
                  </div>
                </div>
                  ...
                  <div className="flex justify-center gap-5">
                    <div
                      onClick={() =>
                        navigate(
                          userInfo
                            ? `/${sellerId}/dashboard/my-wishlist`
                            : "/login"
                        )
                      }
        ...
        >
          <div className="flex justify-start flex-col gap-6">
            <Link to={`/${sellerId}`}>
              <img
                src={
                  supportInfo.logo || "http://localhost:3000/images/logo.png"
                }
                alt="logo"
                className="w-auto h-20 md:h-24"
              />
            ...
            <div className="w-full flex justify-end md-lg:justify-start gap-3 items-center">
              ...
              <div className="flex justify-end flex-col gap-1">
                <h2 className="text-sm font-medium text-slate-700">
                  {supportInfo.mobileNo || "+91 9569555922"}
                </h2>
