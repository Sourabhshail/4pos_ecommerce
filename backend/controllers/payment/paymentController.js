...
class paymentController {
  ...
  payfast_signature = async (req, res) => {
    const { merchant_id, merchant_key, ...data } = req.body;
    ...
    const signatureString = Object.entries(sortedData).map(([key, value]) =>
      `${key}=${encodeURIComponent(value.trim()).replace(/%20/g, "+")}`.join(
        "&"
      )
    );
